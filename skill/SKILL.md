...
| Kernel TUN mode fails on PW4 FW5.16.7 | `/dev/net/tun` exists as a device node, but `tailscaled` in kernel-TUN mode fails to start against it (confirmed via SSH, not assumed). Use userspace-networking + an explicit SOCKS5/HTTP proxy (`--socks5-server`, `--outbound-http-proxy-listen`) instead, and route `curl` through `--proxy http://localhost:<port>`. See DECISIONS.md D04. |
| Killing `tailscaled` drops your own Tailscale SSH session | `tailscale up --ssh` makes the SSH server run *inside* `tailscaled` itself. Any script (or command) that restarts `tailscaled` — including `dash_interactive.sh`'s own proxy bring-up — severs the very SSH connection you're using to run it. The remote command usually still completes if backgrounded with `nohup ... &` before the kill happens, but you won't see confirmation; reconnect with a fresh SSH session afterward rather than assuming failure. Make daemon bring-up idempotent (skip the restart if already healthy) to avoid this in the common case. |
| USB mass-storage mode kills all Kindle background processes | Confirmed empirically: `tailscaled`, `dash_interactive.sh`, `touch_tap` — all suspended (not just paused) the moment you connect via USB for file transfer, and do not resume on their own after eject. Everything needs restarting via KUAL after any USB session. Plan iteration around SSH (once Tailscale SSH is set up) instead of repeated USB remounts where possible. |
| Touch grab held during a slow startup can break swipe-to-unlock | `touch_tap` grabs the touchscreen (`EVIOCGRAB`) early in `dash_interactive.sh`'s startup, before the code that handles sleep/wake (the main interaction loop) is running. If the Kindle's own screen timeout fires during any blocking startup work (e.g. waiting on a network connection) that happens before the main loop starts, the framework never gets touch back and swipe-to-unlock fails. Recovery: kill the orphaned `touch_tap` (`kill $(pgrep touch_tap)`) — the grab releases automatically when its fd closes. Mitigation: keep any blocking pre-main-loop startup work as short as possible; rely on existing fetch fallbacks (cached image, retry-on-next-tap) rather than blocking for full readiness. Hit on hardware 2026-07-31 — see LOG.md. |
