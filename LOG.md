...
## 2026-07-31 — Session: Tailscale integration + live SSH access discovered

- **Discussed:** Build the Tailscale remote-access piece from the README's "Future Directions." Found the Kindle already had a full Tailscale KUAL extension installed (by "Mitanshu") from an earlier, undocumented session — three daemon modes (standard/userspace, SOCKS5+HTTP proxy, kernel TUN), auth key, Tailscale SSH host keys, all pre-built.
- **Found:** the existing identity was joined under a different personal account than the one used for this project. Confirmed with the developer it was their own second account (not a stranger), reset it, installed Tailscale on the Mac, and re-registered the Kindle under the project's own account via a fresh non-ephemeral single-use auth key (generated through the browser, never typed by the assistant).
- **Major workflow unlock:** `start_tailscale.sh` runs `tailscale up --ssh`, so once both devices share a tailnet, **root SSH into the Kindle works directly** (`ssh root@<tailscale-ip>`) — no separate sshd needed. This replaces the USB mass-storage dance for most iteration (USB mode itself was confirmed to kill all background processes including `tailscaled`, so it still has a real cost — needs KUAL taps to restart afterward every time).
- **Settled empirically (not guessed) whether kernel TUN works on this Kindle:** `/dev/net/tun` device node exists, but `start_tailscaled_tun.sh` fails outright — "kernel TUN may not be supported" is correct for this FW5.16.7 build. Userspace-networking + an explicit local SOCKS5/HTTP proxy (`--socks5-server` / `--outbound-http-proxy-listen`) is the only path that lets `curl` route through the tailnet; confirmed via direct testing (plain curl to a 100.x.x.x address times out, `curl --proxy http://localhost:1055 ...` succeeds, sub-second for both the JSON and PNG fetch).
- **Implemented in `dash_interactive.sh`:** new `ensure_tailscale_proxy()` — starts `tailscaled` in proxy mode if the Tailscale extension is installed (purely additive; skipped entirely if it's not), routes `fetch_view()`'s two curl calls through `$CURL_PROXY_ARGS`. `SERVER` is now the Mac's Tailscale IP, not its LAN IP — Tailscale picks a direct LAN path when possible and falls back to DERP relay otherwise, so the same config works both home and away.
- **Iterated on real timing, not assumptions:** a fixed `sleep 3` after starting `tailscaled` fired the first fetch ~13s before DERP actually connected (observed ~20s cold-start), causing a fallback to stale cache. Replaced with polling the server's `/health` through the proxy. Also made it idempotent — skip the restart entirely if the proxy already reaches the server, since bouncing `tailscaled` kills any live Tailscale SSH session too (it's the same process).
- **Incident: touch grab left the Kindle unable to swipe-to-unlock.** Root cause: `touch_tap` grabs the touchscreen (`EVIOCGRAB`) early in startup, but the code that handles sleep/wake doesn't run until after `ensure_tailscale_proxy()`'s connection wait completes. The first version of that wait (bounded at 24s) made the pre-main-loop window long enough that the Kindle's own screen timeout fired inside it — `touch_tap` was left holding the grab with nothing managing sleep/wake yet. Recovered by killing the orphaned `touch_tap` over SSH (grab releases automatically when its fd closes). **Fix:** shortened the wait bound to 6s — `fetch_view()` already falls back to a cached image on failure, and the next tap retries, so a fully-connected guarantee at startup wasn't worth the risk window. Documented as a known limitation, not fully eliminated (a user or timeout-triggered sleep during *any* nonzero pre-main-loop window could still hit this, just far less likely now).
- **Files touched:** spike/dash_interactive.sh (kindle-dash/bin copy synced), LOG.md

## 2026-07-31 — Session: Built the MCP server (D02/D03), generalized for any agent + Kindle

- **Discussed:** Build the MCP server described in DECISIONS.md D02/D03, but generalized so it's not tied to this project's specific agents — usable by anyone with a jailbroken Kindle and an MCP-capable agent.
- **Built:** `mcp_server/` — a real `uv` project (`kindle-dash-mcp`, Python 3.11, `mcp==1.9.4` + Pillow), replacing the stub in `skill/templates/mcp_server_skeleton.py` (deleted — fully superseded):
  - `config.py` — all env-driven: screen width/height, HTTP host/port, data dir, home card cap. PW4 defaults, nothing hardcoded elsewhere.
  - `store.py` — atomic-write disk persistence for the view registry and home-card slots. Slot assignment is dynamic and stable (first-registration order, persisted), not a fixed list of named agents.
  - `renderers.py` — full Pillow implementations of all 5 Phase-1 view types (status_grid, metric_dashboard, text_list, chart_view, progress_view), plus shared header/back/exit/font helpers. Grayscale status encoding (fill darkness + text labels, no color).
  - `http_server.py` — `/view`, `/images`, `/health`, reusing the proven path-sanitization pattern from `spike/view_server.py`.
  - `server.py` — the 4 MCP tools (`update_view`, `push_home_card`, `get_status`, `list_views`) via `FastMCP`, plus `build_home_view()` / `build_agents_overview()` for the dynamically-composed home grid and its overflow view.
  - `__main__.py` — starts the HTTP thread, ensures `home` exists, runs the stdio server.
- **Chose `mcp==1.9.4` over the newly-released `mcp==2.0.0`:** 2.0 restructures `FastMCP` (moved/renamed) and is too new for a server meant to be broadly reusable by agents built against the widely-documented 1.x API. See DECISIONS.md D05.
- **Bug found via `from __future__ import annotations`:** with that import active in `server.py`, `FastMCP`'s context-parameter detection (`issubclass(param.annotation, Context)`) breaks — annotations become strings, and `issubclass()` on a string raises `TypeError: issubclass() arg 1 must be a class`. Removed the future-import from `server.py`; Python 3.11 supports `X | None` natively at runtime anyway, so nothing was lost.
- **Real bug caught during testing, not theoretical:** the Kindle's `hit_test()` awk parser in `dash_interactive.sh` scans `/view` JSON line-by-line and expects each field/`}` on its own line (documented in SKILL.md's "No `jq` on Kindle" section, but easy to miss). `http_server.py`'s first draft used compact `json.dumps(payload)` — parses fine, but every tap would have silently failed to hit (no error, just dead buttons). Fixed to `json.dumps(payload, indent=2)`, confirmed via curl that the `/view` output now matches the proven `spike/view_server.py` format exactly (each tap object's fields and closing `}` each on their own line).
- **Verified (not just written):**
  - All 4 tools called directly in Python: `push_home_card` (2 agents, stable slots 0/1), `update_view` for all 5 view types, `get_status`, `list_views`.
  - All 5 rendered PNGs visually inspected (`Read` tool on the output images) — home grid, metric_dashboard with factor bars + safety-gate banner, text_list with status-colored markers, chart_view with sparkline + baseline, progress_view with bars + stacked bar + legend + items.
  - Error paths: reserved-path rejection (`home`, `system/*`), unknown view type, missing `type` field — all return `{success: false, error: "..."}` instead of raising.
  - HTTP server via curl: `/health`, `/view?path=home` (correct JSON shape), `/images/<name>.png` (200), and a path-traversal attempt `/images/../../../../etc/passwd` (404, not 200 — sanitization holds).
  - **Full real MCP stdio round-trip**, not just unit calls: used `mcp.client.stdio.stdio_client` to actually spawn `uv run kindle-dash-mcp` as a subprocess, `list_tools()` (all 4 present), and `call_tool()` for `push_home_card` and `get_status` — confirms the server speaks correct MCP protocol to a real client, not just that the underlying Python functions work.
- **Docs updated:** `skill/SKILL.md`'s MCP section rewritten from "here's the design" to "here's what's built and where," including the JSON-indent pitfall as a named gotcha. `README.md` — "Not yet built" MCP line removed, new "MCP Server" section replaces "Future Directions" placeholder, repo structure tree updated. `DECISIONS.md` D05 records what was generalized beyond the literal D02/D03 spec (dynamic home slots instead of a fixed named list; env-driven hardware config; the SDK version pin) and why.
- **Hardware verification — done, same session.** The old `spike/view_server.py` was still running from an earlier session and holding port 8888 (`lsof` found it, 2h23m uptime) — killed it, confirmed the port was free, then started `kindle-dash-mcp`'s HTTP layer in its place with the same seeded 2-agent content. (The full `uv run kindle-dash-mcp` entrypoint blocks on `mcp.run(transport="stdio")`, which exits immediately on stdin EOF when no MCP client is attached — expected stdio-transport behavior, not a bug, but not useful for a manual standalone hardware check, so this test ran `http_server.start()` directly instead. The stdio/tool-call path was already verified separately via the real `mcp.client.stdio` round-trip earlier in this session.) Ejected the Kindle's USB volume (`diskutil eject`), the developer unplugged it, and launched **KUAL → Kindle Dashboard → Start Dashboard (interactive)**. Server access log confirms a clean round trip from the Kindle's Tailscale IP: `/health` on connect → `/view?path=home` + `/images/home.png` (home grid rendered) → tapped the Sports Coach card → `/view?path=sports/readiness` + image (metric_dashboard view, correct navigation) → tapped Back → home re-rendered → tapped the Life Coach card → `/view?path=life/nutrition` + image (progress_view view, correct navigation). Both agent cards, both view types, and back-navigation all confirmed working through the new server, using the deployed KUAL bundle unmodified — `menu.json` on-device already pointed at the Mac's current Tailscale IP from the earlier Tailscale session, so no Kindle-side reconfiguration was needed at all.
- **Files touched:** mcp_server/ (new: pyproject.toml, uv.lock, README.md, src/kindle_dash_mcp/{__init__,__main__,config,store,renderers,http_server,server}.py), skill/templates/mcp_server_skeleton.py (deleted), skill/SKILL.md, README.md, DECISIONS.md (D05), LOG.md
